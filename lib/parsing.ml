...
let err_str err =
  ...
  | WrongChar (ex, fnd) ->
      Printf.sprintf "Expected char %s; found %s" (char_to_str ex)
        (char_to_str fnd)
  | WrongString (ex, fnd) ->
      Printf.sprintf "Expected string %s; found %s" ex fnd
  | WrongCharSet (set, fnd) ->
      Printf.sprintf "Expected char in %s; found %s" set (char_to_str fnd)
  | WrongStringSet (set, fnd) ->
      Printf.sprintf "Expected string in %s; found %s" (String.concat ", " set)
        fnd
  | NotInt c -> Printf.sprintf "Expected an int, got %s" (char_to_str c)
  | FailedPred c ->
      Printf.sprintf "Predicate match failed on char %s" (char_to_str c)
...
let ( >>= ) res p =
  match res with Failure e -> Failure e | Success (stack, xs) -> p stack xs

(* Kleisli composition for parse state
   at least, I think that's what it's called *)
let ( >=> ) p1 p2 stack sq = Success (stack, sq) >>= p1 >>= p2

let ( >=>? ) p1 p2 stack sq =
  match p1 stack sq with Failure _ -> p2 stack sq | succ -> succ

let map_error fn p stack sq =
  match p stack sq with Failure e -> Failure (fn e stack sq) | succ -> succ

let push_stack x stack sq = Success (List.cons x stack, sq)

let pop_stack stack sq =
  match stack with [] -> Failure EmptyStack | _ :: xs -> Success (xs, sq)
...
let mod_stack fn stack sq = Success (fn stack, sq)
...
let skip p stack sq = p stack sq >>= mod_stack (fun _ -> stack)
...
let group p stack sq =
  ...
  | Success (ns, nq) ->
      ...
      let group =
        ns_seq
        |> Seq.take_while (fun x -> x != Sentinel)
        |> List.of_seq |> List.rev
      in
      let rest =
        ns_seq |> Seq.drop_while (fun x -> x != Sentinel) |> List.of_seq
      in
      Success (List.cons (Group group) (List.tl rest), nq)
  | e -> e

let repeat n p = Seq.repeat p |> Seq.take n |> Seq.fold_left ( >=> ) nop
...
let expect_letter =
  expect_set "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
...
let stringify_group g =
  ...
  | Group lst ->
      List.to_seq lst
      |> Seq.map (fun cr ->
             match cr with Char c -> c | _ -> failwith "unimplemented")
      |> String.of_seq
  | _ -> failwith "unimplemented"

let unwrap_group g =
  match g with Group lst -> lst | _ -> failwith "Attempted to unwrap a group"
...
let expect_string str =
  String.to_seq str |> Seq.map expect_char |> Seq.fold_left ( >=> ) nop
  |> map_error (fun _ _ sq ->
         WrongString (str, sq |> Seq.take (String.length str) |> String.of_seq))
  |> group
...
let expect_option strlist =
  List.to_seq strlist |> Seq.map expect_string
  |> Seq.fold_left ( >=>? ) fail
  |> map_error (fun e _ _ ->
         match e with
...
let expect_int =
  maybe (expect_char '-')
  >=> at_least_one expect_digit |> group
  |> map_error (fun e _ _ ->
         match e with
...
let expect_list data_p delim_p = expect_nonempty_list data_p delim_p |> maybe
...
let unwrap_int r =
  match r with Int n -> n | _ -> failwith "Tried to unwrap an int"
