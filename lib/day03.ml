...
let tokenize_engine lines =
  let engine_line_tokenizer =
    expect_char '.' >=>? expect_int >=>? any1 |> at_least_one
  in
  List.map
    (fun line -> run_string_parser engine_line_tokenizer line |> unwrap_result)
    lines

type engine_datum = Empty | Number of int | Part of char
type coord = int * int
...
type engine_data = { datum : engine_datum; row : int; col : int }

let process_token_row row_idx row =
  (* processes a row of tokenized input, looking for parts and numbers *)
  (* data contained in the row are marshalled into the engine_data structure *)
  row
  ...
  |> List.fold_left_map
       (fun col_idx token ->
         ...
         (* skip '.' *)
         | Char '.' ->
             (col_idx + 1, { datum = Empty; row = row_idx; col = col_idx })
         (* some kind of part *)
         | Char c ->
             (col_idx + 1, { datum = Part c; row = row_idx; col = col_idx })
         (* number; increment col_idx by the length of the string *)
         | Int n ->
             ( col_idx + String.length (string_of_int n),
...
let process_engine engine_lines =
  (* parses each tokenized line and returns a pair of seqs (parts, numbers) *)
  engine_lines
  |> List.fold_left_map
       (fun row_idx row ->
         (row_idx + 1, process_token_row row_idx row |> List.to_seq))
       0
  (* drop the final value of the accumulator from fold_left_map *)
  |> snd
...
let span_of_int num =
  ...
  | Number n ->
      make_range num.col (num.col + (String.length (string_of_int n) - 1))
...
let create_point_cloud_of_nums =
  Seq.fold_left (fun m n -> add_num_to_point_cloud n m) CoordM.empty
...
let num_border num =
   ...
   * in the span *)
  let all_points =
    CoordS.to_seq num_span |> Seq.flat_map neighbors |> CoordS.of_seq
  in
...
let part_2 lines =
  ...
  let gears =
    Seq.filter
      (fun p -> match p.datum with Part '*' -> true | _ -> false)
      parts
  in
  gears
