...
let get_first_result reslist tlist =
  ...
  | x :: _ -> (
      ...
      | PString s ->
          let _, digit =
            List.find (fun (word, _) -> String.equal s word) tlist
          in
          digit
...
let digify_string str =
  let parser =
    expect_option (List.map fst num_words)
    >=>? expect_digit >=>? skip any1 |> at_least_one
  in
  let rev_parser =
    expect_option (List.map fst num_words_rev)
    >=>? expect_digit >=>? skip any1 |> at_least_one
  in
...
let part_2 lines =
  lines |> List.to_seq |> Seq.map digify_string |> Seq.map int_of_string
  |> Seq.fold_left ( + ) 0
